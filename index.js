...
const modules = ['general', 'administration', 'tickets', 'moderation', 'staff']
modules.forEach(c => {
...
client.on('message', async (message) => {
    ...
    if (message.content === `<@!${client.user.id}>` || message.content === `<@!${client.user.id}>` || message.content === `<@${client.user.id}>` || message.content === `<@${client.user.id}> `) return channel.send(new Discord.MessageEmbed().setColor(config.client.color).setDescription(`Hey! My prefix is \`${config.client.prefix}\`\nUse \`${config.client.prefix}help\` to see all the commands that I can do!`))
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    if (oldMessage.content !== newMessage.content) {
        let channel = newmessage.guild.channels.cache.find(c => c.id === config.channel.logs)
        if (!channel) return
...
client.on('raw', async event => {
        ...
        if (reaction.emoji.name === '✅') {
            ...
            if (member.id !== config.client.owner) {
                if (!reaction.message.guild.member(member).permissions.has('ADMINISTRATOR')) {
                    reaction.remove(member)
                    return reaction.message.guild.member(member).send(new Discord.MessageEmbed().setColor(config.client.color).setDescription('Insufficient Permissions'))
                }
            }
            let embed = new Discord.MessageEmbed()
    ...
    if (reaction.message.channel.id === config.channel.tickets) {
        ...
        if (reaction.emoji.name === '📋') {
            ...
            if (limit === 1) {
                return member.send(new Discord.MessageEmbed().setColor(config.client.color).setDescription('You have reached the maximum amount of tickets opened'))
            } else {
                createChannel()
            }
        }
